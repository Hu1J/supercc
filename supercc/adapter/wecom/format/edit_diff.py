...
def format_diff_markdown(diff_lines: list[DiffLine]) -> str:
    """将 diff_lines 格式化为 markdown diff 代码块。"""
    if not diff_lines:
        return ""
...
class _DiffMarker:
    """通知 message_handler 此工具调用需要渲染彩色 diff。"""
    ...
    def render(self) -> str:
        ...
        diff_text = format_diff_markdown(diff)
        return f"**{file_path}**\n\n{diff_text}"
...
def build_edit_marker(tool_input_json: str) -> _DiffMarker:
    ...
    new_str = data.get("new_string", "")
    diff = colorize_diff(old_str, new_str)
    card = {"file_path": file_path, "diff": format_diff_markdown(diff)}
    return _DiffMarker("Edit", tool_input_json, card)


def build_write_marker(tool_input_json: str) -> list[_DiffMarker]:
    """从 Write 工具的 tool_input JSON 构建 marker list。"""
    data = json.loads(tool_input_json)
    file_path = data.get("file_path", "unknown")
    content = data.get("content", "")
    lines = content.splitlines()
    diff = [DiffLine("insertion", line) for line in lines]
    card = {"file_path": file_path, "diff": format_diff_markdown(diff)}
    return [_DiffMarker("Write", tool_input_json, card)]


class _MemoryCardMarker:
    """记忆工具结果卡片标记 — 触发 markdown 表格渲染。"""
