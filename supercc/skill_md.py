"""Inline skill content for feishu-send-file.
...
description: |
  ...
  调用方式: supercc send <文件路径> --config <config.yaml路径>
  示例: supercc send screenshot.png --config /project/.supercc/config.yaml
  支持图片: png, jpg, jpeg, gif, webp, bmp
  支持文件: pdf, doc, docx, xls, xlsx, zip, txt, csv 等
  config.yaml 路径为当前项目 .supercc/ 目录下的配置文件
---
...
```bash
supercc send /path/to/file.png --config /path/to/.supercc/config.yaml
```

一次可以发送多个文件（并发上传、并发发送，速度更快）：

```bash
supercc send chart.png output.pdf --config /path/to/.supercc/config.yaml
```

## 注意事项

- 使用绝对路径，不要用相对路径
- config.yaml 为当前项目 .supercc/ 目录下的配置文件
