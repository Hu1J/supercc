"""Telegram markdown 优化工具（供 reply_formatter 和 agent_card 共享）。

Telegram 对 MarkdownV2 有严格转义要求，此模块提供文本预处理。
"""
...
def optimize_markdown_style(text: str, card_version: int = 2) -> str:
    """Optimize Markdown for Telegram rendering.

    - Headings: H1-H3 降为 H4，H4-H6 降为 H5（避免 Telegram 解析问题）
    - 代码块保留
    - 表格保留（Telegram MarkdownV2 支持表格）
    - 折叠连续空行
    """
    try:
...
def _optimize_markdown_style_impl(text: str, card_version: int = 2) -> str:
    ...
    if card_version >= 2:
        ...
        # 4. Table spacing — Telegram 支持表格，但需要适当间距
        # 4a: non-table line directly before table row → add blank line first
        r = re.sub(r"^([^|\n].*)\n(\|.+\|)", r"\1\n\n\2", r, flags=re.MULTILINE)
        # 4b: add <br> before table
        r = re.sub(r"\n\n((?:\|.+\|[^\S\n]*\n?)+)", r"\n\n<br>\n\n\1", r)

        # 4c: reduce extra blank lines when non-heading/non-bold text precedes table
        r = re.sub(
            r"^((?!#{4,5} )(?!\*\*).+)\n\n(<br>)\n\n(\|)",
            r"\1\n\2\n\3",
            r,
        ...
        # 4d: bold text before table — keep blank line after bold
        r = re.sub(
            r"^(\*\*.+)\n\n(<br>)\n\n(\|)",
            r"\1\n\2\n\n\3",
            r,
