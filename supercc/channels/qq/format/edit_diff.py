...
def _format_diff_qq_md(diff_lines: list[_BaseDiffLine]) -> str:
    """将 diff_lines 格式化为 QQ markdown 文本，每行带行号（行号右对齐）和颜色标签。"""
    if not diff_lines:
        return ""
...
def format_edit_card(file_path: str, diff_lines: list[_BaseDiffLine]) -> dict:
    """构建 Edit 工具的 QQ diff 卡片（使用 QQ markdown 格式）。"""
    header_md = f"✏️ **Edit** — `{file_path}`"
    diff_md = _format_diff_qq_md(diff_lines)
    content = f"{header_md}\n\n{diff_md}"
    return {
        "content": content,
...
# ----------------------------------------------------------------------
class _DiffMarker:
    """通知 QQCcoreWSClient 此工具调用需要渲染彩色 diff 卡片。"""
    ...
    def __init__(self, tool_name: str, tool_input: str, card: dict):
        ...
        self.card = card              # 预构建的 QQ 卡片 JSON

    def render(self) -> str:
        """渲染为纯文本 diff（verbose 模式使用）。"""
