"""Agent / Codex 响应 QQ 卡片 — 继承 common.format 基类，覆盖 render() 为 QQ 消息格式。"""
from __future__ import annotations
import json

from supercc.channels.common.format.agent import _AgentCardMarker, _CodexMarker
from supercc.channels.qq.format.markdown_util import optimize_markdown_style


class QQAgentCardMarker(_AgentCardMarker):
    """Agent 卡片 — QQ 平台覆盖 render() 输出 QQ 消息格式。"""
...
class QQCodexMarker(_CodexMarker):
    """Codex 卡片 — QQ 平台覆盖 render() 输出 QQ 消息格式。"""
...
def format_agent_card(text: str | dict, title: str = "## 🔀 Agent") -> dict:
    """构建 Agent QQ 卡片（供 core_client.py 直接调用）。"""
...
def format_codex_card(event_type: str, content: str = "", extra: dict | None = None) -> dict:
    """构建 Codex 事件 QQ 卡片（供 core_client.py 直接调用）。"""
