...
- QQ markdown cards for rich content (code blocks, tables)
"""
...
from supercc.channels.qq.format.edit_diff import build_edit_marker, build_write_marker, _DiffMarker
from supercc.channels.qq.format.markdown_util import optimize_markdown_style, _count_tables_outside_code_blocks
from supercc.channels.qq.format.questionnaire_card import _AskUserQuestionMarker
from supercc.channels.qq.format.agent_card import QQAgentCardMarker, QQCodexMarker

QQ_MAX_MESSAGE_LENGTH = 4000
# QQ CardKit limit for markdown tables per card
QQ_CARD_TABLE_LIMIT = 230099

def should_use_card(text: str) -> bool:
    """Decide whether to send as QQ Card vs plain text.

    Cards are used for content with fenced code blocks or markdown tables
    (better rendering in a wide-screen card). Falls back to plain text if there
    are too many tables.
    """
    table_count = _count_tables_outside_code_blocks(text)
    if table_count > QQ_CARD_TABLE_LIMIT:
        return False
...
class QQReplyFormatter:
    ...
    def format_tool_call(
        ...
        **kwargs,
    ) -> str | _DiffMarker | list[_DiffMarker] | MemoryCardMarker | _AskUserQuestionMarker | QQAgentCardMarker | QQCodexMarker:
        ...
        # Memory MCP tools → 卡片标记（触发 QQ 消息卡片）
        ...
        # Cron MCP tools → ⏰ 时钟图标
            ...
            if tool_input and len(tool_input) <= QQ_MAX_MESSAGE_LENGTH - len(msg) - 5:
                msg += f"\n`{tool_input}`"
            return msg
        ...
        if tool_input:
            if len(tool_input) <= QQ_MAX_MESSAGE_LENGTH - len(msg) - 5:
                msg += f"\n`{tool_input}`"
            else:
    ...
    def _format_agent_tool(self, tool_input: str) -> QQAgentCardMarker:
        """Format Agent (sub-agent) tool call → QQAgentCardMarker。"""
        return QQAgentCardMarker("Agent", tool_input)

    def _format_codex_tool(self, tool_input: str) -> QQCodexMarker:
        """Format mcp__codex__codex tool call → QQCodexMarker。"""
        try:
        ...
        return QQCodexMarker(event_type, prompt, extra, tool_input)
