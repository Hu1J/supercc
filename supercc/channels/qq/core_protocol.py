"""消息转换：QQ IncomingQQMessage ↔ 核心 InboundMessage / OutboundMessage."""
...
def incoming_to_inbound(
    ...
    """
    将 QQ IncomingQQMessage 转换为核心 InboundMessage。

    bot_id: 从配置读取的QQ机器人 open_id
    ...
    # 消息类型
    msg_type_map = {
        "C2C_MESSAGE_CREATE": MessageType.TEXT,
        "GROUP_AT_MESSAGE_CREATE": MessageType.TEXT,
    }
    ...
    return InboundMessage(
        ...
        media_path=None,
        user_open_id=incoming.author_id or None,
        thread_id=None,
        timestamp=_cst_now(),
        extra={
            "raw": incoming.raw,
            "is_group_chat": bool(incoming.group_openid),
            "mention_bot": False,
            "mention_ids": [],
            "group_name": incoming.group_name,
            "chat_type": incoming.message_type,
...
def outbound_to_renderable(
    ...
    """
    将核心 OutboundMessage 转换为QQ可渲染格式。

    reply_to_message_id: 用于回复同一消息（引用）
    """
    from supercc.channels.qq.format.reply_formatter import should_use_card
